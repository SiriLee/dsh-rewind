 ...
 * settings-namespace read that lets the host bundle link and run on BOTH DSH 0.1.1
 * (which exports the `settingsNamespace` brand helper) and 0.1.2 (which
 * removed it).
 *
 ...
 * 0.1.2 (the helper is gone). These tests pin the dual-version behavior of the
...
describe('readSettingsSection', () => {
  it('reads the section through the 0.1.1 brand helper', () => {
    const provider: SettingsProviderLike = {
      get: ns => (ns === 'locale' ? { preference: 'zh' } : undefined),
    }
    // 0.1.1 `settingsNamespace('locale')` returns `'locale'` (the brand is erased
  ...
  it('reads the section by raw namespace when the helper is absent (0.1.2)', () => {
    const provider: SettingsProviderLike = {
      get: ns => (ns === 'locale' ? { preference: 'en' } : undefined),
    }
    // 0.1.2 removed the brand helper, so the caller passes undefined and the
