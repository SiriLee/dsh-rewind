...
import { defaultProbe, reconcileTracked, SnapshotStore, isLinkEntry, type DiskProbe } from '../src/snapshot.ts'
...
describe('reconcileTracked (user-message boundary snapshot)', () => {
  ...
  it('records every message; an unchanged file becomes a content link', async () => {
    const file = await touch('tracked.txt', 'original')
    await store.recordEntry(session, { callId: 'tool', anchorSeq: 5, path: file, before: 'original' })
    await writeFile(file, 'edited by turn', 'utf8')
    const tracked = await store.trackedPaths(session)

    // First sighting: unconditionally record the current state.
    expect(await reconcileTracked(store, session, 7, tracked)).toBe(1)
    // Next boundary: state unchanged → still RECORDS, but as a content LINK
    // (Option B: every boundary is a complete recording point, so the exact
    // state at this message stays recoverable — never a zero-record gap).
    expect(await reconcileTracked(store, session, 8, tracked)).toBe(1)
    expect((await store.entriesAfter(session, 8)).some(isLinkEntry)).toBe(true)
    // External change → recorded as a real entry again.
    await writeFile(file, 'externally edited', 'utf8')
    expect(await reconcileTracked(store, session, 9, tracked)).toBe(1)
  })

  it('restores the pre-external state when rewinding to a no-change boundary', async () => {
    const file = await touch('tracked.txt', 'original')
    // Seq 5 turn: a write-class edit modifies the file (before = 'original').
    await store.recordEntry(session, { callId: 'tool', anchorSeq: 5, path: file, before: 'original' })
    await writeFile(file, 'X', 'utf8')
    const tracked = await store.trackedPaths(session)

    // Seq 6 (a reply that changed nothing): the boundary records X. Under the
    // old change-only model this boundary was SKIPPED, losing the state at the
    // reply — rewinding to it then returned the post-external state instead.
    expect(await reconcileTracked(store, session, 6, tracked)).toBe(1)
    // An external edit (e.g. bash) between seq 6 and seq 7: X → Z.
    await writeFile(file, 'Z', 'utf8')
    expect(await reconcileTracked(store, session, 7, tracked)).toBe(1)

    // Rewind to the no-change reply (seq 6) restores X — the pre-external
    // state the write-class capture never saw.
    await writeFile(file, 'current', 'utf8')
    const atReply = await store.restoreAfter(session, 6, unlink)
    expect(atReply.restored).toEqual([file])
    expect(await readFile(file, 'utf8')).toBe('X')
    // Rewind to seq 7 restores Z (the state as of that boundary).
    await writeFile(file, 'current2', 'utf8')
    const atEdit = await store.restoreAfter(session, 7, unlink)
    expect(atEdit.restored).toEqual([file])
  ...
  it('skips a failing probe per-file and still records the rest', async () => {
    const good = await touch('good.txt', 'good')
    const bad = await touch('bad.txt', 'bad')
    await store.recordEntry(session, { callId: 'good', anchorSeq: 5, path: good, before: 'good' })
    await store.recordEntry(session, { callId: 'bad', anchorSeq: 5, path: bad, before: 'bad' })
    const tracked = await store.trackedPaths(session)
    const probe: DiskProbe = {
      readText: async (path) => {
        if (path === bad) throw new Error('boom')
        return await defaultProbe.readText(path)
      },
      isLink: (path) => defaultProbe.isLink(path),
    }
    // The bad path is skipped (probe failure), the good path still records.
    expect(await reconcileTracked(store, session, 7, tracked, probe)).toBe(1)
    expect((await store.entriesAfter(session, 7)).some((e) => e.path === good)).toBe(true)
  })
})

describe('trackedPathsFromLatestAnchor (newest-group snapshot)', () => {
  it('returns the newest non-empty group as the current tracking set', async () => {
    const a = await touch('a.txt', 'A')
    const b = await touch('b.txt', 'B')
    // The latest group (7) is a complete snapshot of both tracked files.
    await store.recordEntry(session, { callId: 'a7', anchorSeq: 7, path: a, before: 'A' })
    await store.recordEntry(session, { callId: 'b7', anchorSeq: 7, path: b, before: 'B' })
    // An older group (5) holds only a — it must NOT win over the newest group.
    await store.recordEntry(session, { callId: 'a5', anchorSeq: 5, path: a, before: 'x' })
    expect(await store.trackedPathsFromLatestAnchor(session)).toEqual(new Set([a, b]))
  })

  it('matches the union when the newest group is a full snapshot (links included)', async () => {
    const a = await touch('a.txt', 'A')
    await store.recordEntry(session, { callId: 'a7', anchorSeq: 7, path: a, before: 'A' })
    const tracked = await store.trackedPaths(session)
    // A boundary recheck of the same content at seq 8 dedups to a LINK; the
    // newest group still enumerates the track by the link's `path`.
    expect(await reconcileTracked(store, session, 8, tracked)).toBe(1)
    expect(await store.trackedPathsFromLatestAnchor(session)).toEqual(await store.trackedPaths(session))
...
describe('content dedup (in-place link; old + new entry format)', () => {
  ...
  it('collapses two identical-content records into one real + one link', async () => {
    const file = await touch('a.txt', 'v')
    // Two recorders capturing the same `before` collapse to one real + one
    // link (recordEntry content dedup) — the mechanism that keeps every
    // message's unchanged file cheap.
  ...
  it('a write-class register then a boundary snapshot dedups to one real + one link', async () => {
    const file = await touch('a.txt', 'A')
    // Turn 1 (seq 5): write-class registers the file (before = 'A'); file → X.
    await store.recordEntry(session, { callId: 'tool', anchorSeq: 5, path: file, before: 'A' })
    await writeFile(file, 'X', 'utf8')
    const tracked = await store.trackedPaths(session)
    // Turn 2 boundary (seq 6): records X as a real entry (differs from 'A').
    expect(await reconcileTracked(store, session, 6, tracked)).toBe(1)
    // Turn 3 boundary (seq 7): still X (unchanged) → LINK to seq 6's X.
    expect(await reconcileTracked(store, session, 7, tracked)).toBe(1)
    // Composition: A@5 (register real), X@6 (real), X@7 (link).
    const entries = await store.entriesAfter(session, 5)
    expect(entries.filter(isLinkEntry)).toHaveLength(1)
    expect(entries.filter(e => !isLinkEntry(e))).toHaveLength(2)
    // Rewinding to turn 2 restores X (the state as of that boundary).
