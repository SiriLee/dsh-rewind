...
describe('session-log-io', () => {
  ...
  it('decodeEventBody expands packed rows and range-encoded provenance', () => {
    // Build a body from sampleEvents, then decode it back to a fully dense list.
    const buffer = encodeSessionLog(HEADER, sampleEvents())
    const text = decodeZstd(buffer)
  ...
  it('round-trips {type,seq,data} losslessly through encode→decode', () => {
    const input = sampleEvents()
    const buffer = encodeSessionLog(HEADER, input)
    const text = decodeZstd(buffer)
    const { body } = splitSession(text)
    const events = decodeEventBody(body)
  ...
  it('produces exactly 2 frames with the header in its own frame', () => {
    const buffer = encodeSessionLog(HEADER, sampleEvents())
    const { frames } = scanZstdFrames(buffer)
    expect(frames).toHaveLength(2)
    // Frame 0 decodes to exactly one header line + newline.
    const frame0 = decodeZstd(buffer.subarray(frames[0]!.start, frames[0]!.end))
    expect(frame0).toBe(`${HEADER}\n`)
  })

  it('rejects a corrupt zstd magic', () => {
    const buffer = encodeSessionLog(HEADER, sampleEvents())
    buffer.writeUInt32LE(0xdeadbeef, 0)
    expect(() => scanZstdFrames(buffer)).toThrow(/bad magic/)
  })

  it('preserves the header line verbatim (no re-serialization drift)', () => {
    const buffer = encodeSessionLog(HEADER, sampleEvents())
    const text = decodeZstd(buffer)
