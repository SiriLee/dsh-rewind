...
策略持久化到 `<dsh home>/snapshot-cleanup.json`：
...
24 小时窗口锚定在**持久化**的上次清扫时间（`<dsh home>/snapshot-cleanup-last-sweep.json`）上，因此宿主重启不会重置：自动清扫在**每次运行只检查一次**，于某个窗口的首次会话活动（一条用户消息或一次工具调用完成）时执行，且仅在**已启用**且距上次清扫 **>=24 小时**时才清理。它后台运行，从不阻塞触发它的活动。由于每次运行只检查一次，想让改动立即生效最好用 `/snapshot-auto-cleanup run`；手动编辑配置文件（或在本次运行的首次活动之后才开启）会到下次运行才生效。
