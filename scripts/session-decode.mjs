 ...
 * utility: it never touches `~/.dsh` and never modifies the input.
 *
 * Usage:
 *   node scripts/session-decode.mjs <input.zstd>            # print plaintext to stdout
 *   node scripts/session-decode.mjs -o <out.txt> <input.zstd>  # write plaintext to <out.txt>
 *   node scripts/session-decode.mjs --stats <file...>       # per-file: bytes / frames / lines, no body
 *
 * Decoding uses Node ≥ 22 native `node:zlib` zstd primitives (no deps).
 */
...
/** Decompress a file path; returns { text, bytes, frames, tornStart }. */
...
async function main() {
  ...
  // `-o <out>` writes the plaintext to a file; otherwise stdout.
  let out = null
  let files = args
  if (args[0] === '-o') {
    if (args.length < 2) {
      console.log('usage: node scripts/session-decode.mjs [-o out.txt] <input.zstd>')
      process.exit(1)
    }
    out = args[1]
    files = args.slice(2)
  ...
  const src = files[0]
  try {
    const { text } = await decodeFile(src)
    if (out !== null) {
      await writeFile(out, text)
      console.log(`wrote ${src} -> ${out} (${text.length} chars)`)
    } else {
      process.stdout.write(text)
      if (!text.endsWith('\n')) process.stdout.write('\n')
    }
  } catch (e) {
    process.stderr.write(`${src}: [decode fail] ${e.message}\n`)
    process.exit(1)
  }
}
