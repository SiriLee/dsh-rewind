...
export const en = {
  ...
  'cleanup.clearDry': 'Dry-run: would clear {entries} snapshot(s) across {anchorGroups} anchor group(s) ({journals} journal(s)) of the current session, freeing {bytes} bytes. Re-run with --apply to delete.',
  'cleanup.clearApply': 'Cleared {entries} snapshot(s) across {anchorGroups} anchor group(s) ({journals} journal(s)) of the current session, freeing {bytes} bytes. This session now records snapshots fresh from its current state.',
...
export const zh: Record<HostKey, string> = {
  ...
  'cleanup.clearDry': '预演：将清除当前会话的 {entries} 个快照（共 {anchorGroups} 个 anchor 组、{journals} 个日志），释放 {bytes} 字节。加 --apply 正式删除。',
  'cleanup.clearApply': '已清除当前会话的 {entries} 个快照（共 {anchorGroups} 个 anchor 组、{journals} 个日志），释放 {bytes} 字节。该会话已重置为从当前状态重新记录快照。',
