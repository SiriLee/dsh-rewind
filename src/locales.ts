...
export const en = {
  ...
  'rewindfix.description': 'Rewrite legacy rewind markers (A/B) in closed sessions to the current form-C shape',
  'rewindfix.inputHint': 'no args = dry-run preview; --apply = execute (takes minutes)',
  'rewindfix.dryRun': '=== dry-run: {scan} sessions scanned / {will} will be repaired / {skip} skipped / {fail} failed ({secs}s) ===',
  'rewindfix.done': '=== done: {repair} repaired / {skip} skipped / {fail} failed ({secs}s) ===',
  'rewindfix.cancelled': '=== cancelled ===',
  'rewindfix.launcherGuard': '! This session itself needs repair; it is open and cannot be modified in place. Start a NEW session (or close it) — a closed session is repaired when no window holds it.',
  ...
  'rewindfix.fail.repair': 'FAIL (repair) {error}',
...
export const zh: Record<HostKey, string> = {
  ...
  'rewindfix.description': '将已关闭会话中的旧版 rewind 标记（A/B）改写为当前 form-C 形态',
  'rewindfix.inputHint': '无参 = dry-run 预览；--apply = 执行（需数分钟）',
  'rewindfix.dryRun': '=== 预演：扫描 {scan} 个会话 / 将修复 {will} 个 / 跳过 {skip} 个 / 失败 {fail} 个（{secs}s）===',
  'rewindfix.done': '=== 完成：修复 {repair} 个 / 跳过 {skip} 个 / 失败 {fail} 个（{secs}s）===',
  'rewindfix.cancelled': '=== 已取消 ===',
  'rewindfix.launcherGuard': '! 你正在一个本身需要修复的会话里运行；它处于打开状态，无法原地修改。请新建一个会话（或关闭它）——关闭的会话会在没有窗口持有它时被修复。',
  'rewindfix.counts': 'A={a} B={b} → C={c}',
  ...
  'rewindfix.fail.repair': '失败（修复）{error}',
