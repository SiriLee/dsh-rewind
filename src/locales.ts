...
export const en = {
  ...
  'cleanup.runDry': 'Dry-run: would remove {deleted} session snapshot backup(s), freeing {freed} bytes; {kept} kept, {remaining} bytes remain.',
...
export const zh: Record<HostKey, string> = {
  ...
  'cleanup.runDry': '预演：将删除 {deleted} 个会话的快照备份，释放 {freed} 字节；保留 {kept} 个，剩余 {remaining} 字节。',
